import { ArrowDownRight, ArrowUpRight, Download, Plus, Sparkles } from "lucide-react";
...
export default function DashboardPage() {
  return <AppShell><div className="mx-auto max-w-[1600px] px-4 py-7 sm:px-7 lg:px-10 lg:py-9">
    <div className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><div className="mb-2 flex items-center gap-2 text-sm font-semibold text-forest"><Sparkles size={15}/>今日经营决策</div><h1 className="text-3xl font-bold tracking-[-0.035em] sm:text-[34px]">早上好，MJC。</h1><p className="mt-2 text-base text-[#6f7b74]">从选品到广告，把今天最值得处理的机会放在这里。</p></div><div className="flex gap-2"><button className="flex h-10 items-center gap-2 rounded-lg border border-[#d4ded8] bg-white px-3.5 text-sm font-semibold shadow-sm"><Download size={16}/>导出周报</button><button className="flex h-10 items-center gap-2 rounded-lg bg-forest px-4 text-sm font-semibold text-white shadow-sm hover:bg-[#0b5a41]"><Plus size={17}/>新建研究</button></div></div>
